...
def transmissivity(ε, σ, f, depths):
    ...
    """
    Δd = np.hstack(([0], np.diff(depths)))
    k_0 = 2.0 * π / (3.0e8 / f)
    k = prop_constant(ε, σ, f)
    return np.exp(-1.0j * k_0 * Δd + 1.0j * k * Δd)
...
def incident_to_scatterer(E,
    ...
    """
    prop = np.exp(1.0j * k_0 * depths) / (4.0 * π * depths)
...
def fujita_model_TS(E_transmit, depths, obs_angles, T, S, iso_angles, k_0):
    """Run the Fujuta model assumuing we already have T_i and S_i."""
    return_matrix = np.zeros((depths.shape[0], obs_angles.shape[0], 2), dtype=np.complex128)
    for i, angle in enumerate(obs_angles):
        θs = iso_angles - angle
        return_matrix[:, i, :] = observed_from_scaterrer(
            scattered(
...
def fujita_model_εS(E_transmit, depths, obs_angles,
    ...
    here.
    """
    T = np.zeros((depths.shape[0], 2, 2), dtype=np.complex128)
    if len(ε.shape) > 1:
...
def ϕ(ε_x, ε_y, f, depths):
    ...
    if hasattr(ε_x, 'shape') and len(ε_x.shape) > 0:
        return np.hstack(([0], 4.0 * π * f / C * np.cumsum(np.diff(depths) * (np.sqrt((ε_x[1:] + ε_x[:-1]) / 2.)
                                                                                - np.sqrt((ε_y[1:] + ε_y[:-1]) / 2.)))))
    else:
...
def brisbourne_2019_plots(debug=False):
    ...
    # A_xy = np.array([[0.25, 0.25], [0.25, 0.25]])
    npts = 2000
    A_xy = np.zeros((npts, 2, 2))
    A_xy[:, 0, 0] = 0.0
    A_xy[:, 1, 1] = 0.3333
    φ_fab, ε = φ_and_ε(A_xy)
    ...
    # iso_power = fujita_model_εS(E_transmit, depths, obs_angles,
    #                             np.ones((2,)) * ε_prime, S, φ_fab, freq)
    ...
    cm = axcb.imshow(angle(power, obs_angles),
                     extent=extent_angle,
                     vmin=-alim, vmax=alim)
    axcb.set_aspect(aspect="auto")
    axcb.set_title('Angle')
    # plt.colorbar(cm, cax=axccb)
    ...
    axps.set_aspect(aspect="auto")
    axps.set_ylabel('Depth (m)')
    cm = axcs.imshow(angle(power_anisoscatter, obs_angles),
                     extent=extent_angle,
                     vmin=-alim, vmax=alim)
    axcs.set_aspect(aspect="auto")
    # plt.colorbar(cm, cax=axccs)
    # axccs.set_label('Angle (degrees)')
    ...
    axpa.set_xticklabels(['0', r'$\frac{\pi}{4}$', r'$\frac{\pi}{2}$',
                          r'$\frac{3\pi}{4}$', r'$\pi$'])
    axpa.set_xlabel('Antenna orientation θ\n(radians)')
    cm = axca.imshow(angle(power_bothaniso, obs_angles),
                     extent=extent_angle,
                     vmin=-alim, vmax=alim)
